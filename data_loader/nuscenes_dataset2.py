...
class NuscenesDataset2(Dataset):
    ...
    def __init__(self, version: str = 'v1.0-mini',dataset_dir: str = '../../data/nuscenes-mini/v1.0-mini', 
                ...
                frame_count=2,target_image_size=(256, 512), dilation=1, offset_d=0,
    ...
    def format_intrinsics(self,intrinsics, target_image_size):
        ...
        intrinsics_mat[0, 0] = intrinsics[0] * target_image_size[1]   # fx*w
        intrinsics_mat[1, 1] = intrinsics[1] * target_image_size[0]   # fy*h
    ...
    def get_depth_map(self,new_pts,new_depth,target_image_size):
        '''
        把lidar点都打到depth map上, 并且转化成inv depth map
        Input
            new_pts - (3,n)
            new_depth - (n,)
    ...
    def compute_target_intrinsics(self, key_intrinsic_K, orig_size, target_image_size):
        ...
        if r_orig >= r_target: # 原图比target 竖直方向更长，即1600都取到了
            ...
            c_x = P_cam[0, 2] / orig_size[1]    # tx/1600
            c_y = (P_cam[1, 2] - (orig_size[0] - new_height) / 2) / new_height  # (ty-50)/800

            rescale = orig_size[1] / target_image_size[1]  # 放缩系数  1600/512     ～      800/256

        else:   # 原图比target 更宽，即竖直的800都取到了
        ...
        f_x = P_cam[0, 0] / target_image_size[1] / rescale  # fx * w/target_h = 初始的fx/w     => fx / 512 / (1600/512)
        f_y = P_cam[1, 1] / target_image_size[0] / rescale     # 即fy/800     =>  fy / 256 / (800/256)
